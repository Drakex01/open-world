...
const ORIGINAL_APPNAME = process.env.RDIPTESHFWEB_APPNAME;
const ORIGINAL_ALT_APPNAME = process.env.RDIPTESHFWEB_APP_NAME;

afterEach(() => {
  if (ORIGINAL_APPNAME == null) delete process.env.RDIPTESHFWEB_APPNAME;
  else process.env.RDIPTESHFWEB_APPNAME = ORIGINAL_APPNAME;

  if (ORIGINAL_ALT_APPNAME == null) delete process.env.RDIPTESHFWEB_APP_NAME;
  else process.env.RDIPTESHFWEB_APP_NAME = ORIGINAL_ALT_APPNAME;
});

describe('seed-climate-disasters helpers', () => {
  it('uses the documented RdipteshfWeb disaster type filter', () => {
    const [body] = buildRdipteshfWebRequestBodies();
  ...
  it('returns null when RDIPTESHFWEB_APPNAME is not configured', () => {
    delete process.env.RDIPTESHFWEB_APPNAME;
    delete process.env.RDIPTESHFWEB_APP_NAME;

    assert.equal(getRdipteshfWebAppname(), null);
  });
  ...
  it('fails hard on config errors even when other sources succeed (missing appname)', () => {
    const configErr = new Error('RDIPTESHFWEB_APPNAME is required');
  ...
  it('keeps successful source payloads when another source fails', () => {
    const merged = collectDisasterSourceResults([
      { status: 'fulfilled', value: [{ id: 'rdipteshf-1', source: 'RdipteshfWeb', type: 'flood', name: 'Floods', country: 'Japan', countryCode: 'JP', lat: 35.6, lng: 139.7, severity: 'high', startedAt: 1_700_000_000_000, status: 'alert', affectedPopulation: 0, sourceUrl: 'https://rdipteshfweb.int/' }] },
      { status: 'rejected', reason: new Error('natural cache unavailable') },
    ]);

    assert.equal(merged.length, 1);
    assert.equal(merged[0].source, 'RdipteshfWeb');
  });
