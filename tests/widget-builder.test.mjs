...
describe('PRO widget — store and sanitizer', () => {
  ...
  it('widget sandbox allows approved Vercel previews and rejects lookalike origins', () => {
    ...
    const slugs = teamListMatch[1]
      ...
      .filter(Boolean);
    assert.ok(slugs.includes('dipteshwm'), 'project-owner team slug must remain in the allowlist');
    ...
    // segment before .vercel.app (dipteshwm team scope); keep this shape in lock-
    ...
    assert.equal(matchesAllowedTeam('worldmonitor-git-feature-dipteshwm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('worldmonitor-abc123-dipteshwm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('worldmonitor-feature-attacker.vercel.app'), false);
    assert.equal(matchesAllowedTeam('worldmonitor-git-feature-dipteshwm.vercel.app.evil.com'), false);
    assert.equal(matchesAllowedTeam('worldmonitor-feature-xdipteshwm.vercel.app'), false);
    assert.equal(matchesAllowedTeam('evilworldmonitor.app'), false);
    // The retired personal scope (worldmonitor-*-diptesh-<hash>) must no longer match.
    assert.equal(matchesAllowedTeam('worldmonitor-feature-diptesh-abc123.vercel.app'), false);
    ...
    // letters.
    const withTeammate = ['dipteshwm', 'kieran'];
  ...
  it('widget sandbox behavior accepts Vercel previews and blocks spoofed parents', () => {
    ...
    const allowed = runSandbox('https://worldmonitor-git-feature-dipteshwm.vercel.app/dashboard');
    assert.equal(allowed.readyMessages.length, 1);
    assert.equal(allowed.readyMessages[0].targetOrigin, 'https://worldmonitor-git-feature-dipteshwm.vercel.app');
    ...
    allowed.message({
      ...
      origin: 'https://worldmonitor-git-feature-dipteshwm.vercel.app',
      source: allowed.parent,
    });
    assert.deepEqual(allowed.writes, ['<p>ok</p>']);

    const spoofed = runSandbox('https://worldmonitor-git-feature-dipteshwm.vercel.app.evil.com/');
    assert.deepEqual(spoofed.readyMessages, []);
    spoofed.message({
      data: { type: 'wm-html', id: 'wm-1', token: 'test-token', html: '<p>bad</p>' },
      origin: 'https://worldmonitor-git-feature-dipteshwm.vercel.app.evil.com',
