...
// The Vercel project moved from the personal scope (worldmonitor-*-diptesh-<hash>)
// to the "dipteshwm" team scope. Browsers send Origin on the POST to
...
describe('isAllowedOrigin — Vercel preview allowlist (dipteshwm team scope)', () => {
  ...
  const ALLOWED = [
    ['git-branch alias URL', 'https://worldmonitor-git-feature-dipteshwm.vercel.app'],
    ['hash deployment URL', 'https://worldmonitor-abc123def456-dipteshwm.vercel.app'],
    ['apex production origin', 'https://worldmonitor.app'],
    ['production subdomain', 'https://tech.worldmonitor.app'],
  ];

  const REJECTED = [
    ['non-worldmonitor vercel.app origin', 'https://some-other-app-dipteshwm.vercel.app'],
    ...
    ['suffix-spoofed dipteshwm origin', 'https://worldmonitor-git-feature-dipteshwm.vercel.app.evil.com'],
    ['dead personal-scope preview (post-migration)', 'https://worldmonitor-feature-diptesh-abc123.vercel.app'],
  ];
...
describe('CORS triplet parity — dipteshwm preview pattern stays tight in all three twins', () => {
  ...
  // (1) the dipteshwm-scoped preview pattern is present, and
  ...
  for (const rel of TWINS) {
    it(`${rel} scopes Vercel previews to the dipteshwm team`, async () => {
      const source = await readFile(new URL(rel, import.meta.url), 'utf8');
      assert.ok(
        source.includes('-dipteshwm\\.vercel\\.app'),
        `${rel} must allow worldmonitor-*-dipteshwm.vercel.app previews`,
      );
      assert.ok(
...
describe('CORS Worker superset invariant — edge allowlist ⊇ function allowlist', () => {
  ...
  // function↔Worker drift (the bug that left dipteshwm previews dark).
  //
  ...
  const PROD_ORIGINS = [
    ...
    'https://worldmonitor-git-feature-dipteshwm.vercel.app',
    'https://worldmonitor-abc123def456-dipteshwm.vercel.app',
    'tauri://localhost',
    'asset://localhost',
    // Negatives — the function rejects these, so the superset assertion is a
    // no-op for them; included to document the boundary.
    'https://some-other-app-dipteshwm.vercel.app',
    'https://worldmonitor-git-feature-attacker.vercel.app',
    'https://worldmonitor-feature-diptesh-abc123.vercel.app',
    'https://evil.com',
  ];
