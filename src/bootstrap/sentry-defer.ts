...
// to `api.cyberspace.app` and the self-hosted R2 PMTiles bucket are NOT
...
const THIRD_PARTY_FETCH_HOST_ALLOWLIST = new Set([
  ...
  // itself (`retryImmediately`); a `Failed to fetch (clerk.cyberspace.app)`
  ...
  // ignoreError. NOT our `api.cyberspace.app`, which stays off the list so
  // genuine API regressions still surface (WORLDMONITOR-SA/SB).
  'clerk.cyberspace.app',
]);
...
function buildSentryInitOptions(): Parameters<SentryNs['init']>[0] {
  ...
  return {
    ...
    release: `cyberspace@${__APP_VERSION__}`,
    environment: (location.hostname === 'cyberspace.app' || location.hostname.endsWith('.cyberspace.app')) ? 'production'
      : location.hostname.includes('vercel.app') ? 'preview'
      : 'development',
    enabled: Boolean(sentryDsn) && !location.hostname.startsWith('localhost') && !('__TAURI_INTERNALS__' in window),
    allowUrls: [
      /https?:\/\/(www\.|tech\.|finance\.|commodity\.|happy\.)?cyberspace\.app/,
      /https?:\/\/.*\.vercel\.app/,
    ],
    sendDefaultPii: true,
    ...
    beforeSend(event) {
      ...
      // bucket, `api.cyberspace.app`) are intentionally NOT in the set so a
