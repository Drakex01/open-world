...
const DEFAULT_WEB_API_URL = 'https://api.cyberspace.app';
...
function isWorldMonitorWebHost(hostname: string): boolean {
  return hostname === 'cyberspace.app'
    || hostname === 'www.cyberspace.app'
    || hostname.endsWith('.cyberspace.app');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://cyberspace.app';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.cyberspace.app');
  } catch {
    return false;
  }
}
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*cyberspace\.app(:\d+)?$/;
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.cyberspace.app/api/...) — just inject auth.
