...
import { fetchLiveVideoInfo, fetchRecentVideos, RecordedVideo } from '@/services/live-news';
import { isDesktopRuntime, getRemoteApiBaseUrl, getApiBaseUrl, getLocalApiPort, toApiUrl } from '@/services/runtime';
...
export interface LiveChannel {
  ...
  channelId?: string; // YouTube channel ID for RSS uploads feed
}


// Full variant: World news channels (24/7 live streams)
const FULL_LIVE_CHANNELS: LiveChannel[] = [
  { id: 'india-today', name: 'India Today', handle: '@indiatoday', channelId: 'UC5AEoLd35g3CoQON1E16H7g', fallbackVideoId: 'sYZtOFzM78M' },
  { id: 'ndtv', name: 'NDTV 24x7', handle: '@NDTV', channelId: 'UC4R86hb57yxyU1Ms9j1Gfgg', fallbackVideoId: 'uY2nJa291W8' },
  { id: 'bloomberg', name: 'Bloomberg', handle: '@markets', channelId: 'UCo3W5dRLd3Kee5d03aN20Sg', fallbackVideoId: 'iEpJwprxDdk' },
];
...
export class LiveNewsPanel extends Panel {
  ...
  private layoutWrapper: HTMLDivElement | null = null;
  private recordedVideosContainer: HTMLDivElement | null = null;
  private recentVideos: RecordedVideo[] = [];
  ...
  private get embedOrigin(): string {
    ...
    try { return new URL(getRemoteApiBaseUrl()).origin; } catch { return 'https://cyberspace.app'; }
  }
  ...
  private setupBridgeMessageListener(): void {
    this.boundMessageHandler = (e: MessageEvent) => {
      ...
      if (msg.type === 'yt-ready') {
        ...
        if ((code === 153 || code === 150 || code === 101) && this.activeChannel.fallbackVideoId &&
  ...
  private createChannelSwitcher(): void {
    ...
    // Search YouTube, URL or Video ID bar
    const searchContainer = document.createElement('div');
    searchContainer.className = 'live-news-search-container';
    searchContainer.style.cssText = 'display: flex; align-items: center; gap: 4px; margin-left: 12px; margin-right: 8px; flex-shrink: 0; align-self: center;';

    const searchInput = document.createElement('input');
    searchInput.type = 'text';
    searchInput.className = 'live-news-search-input';
    searchInput.placeholder = 'Enter YouTube URL, Video ID or query...';
    searchInput.style.cssText = 'padding: 4px 8px; font-size: 11px; background: var(--input-bg, #1a1a1a); border: 1px solid var(--border); color: var(--text); border-radius: 4px; width: 220px; height: 24px; outline: none; transition: border-color 0.2s;';
    searchInput.addEventListener('focus', () => {
      searchInput.style.borderColor = 'var(--accent-color, #00f0ff)';
    });
    ...
    searchBtn.className = 'live-news-search-btn';
    searchBtn.style.cssText = 'padding: 4px 8px; font-size: 11px; height: 24px; background: var(--surface); border: 1px solid var(--border); color: var(--text); border-radius: 4px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center;';
    searchBtn.textContent = 'Play';

    searchContainer.appendChild(searchInput);
    searchContainer.appendChild(searchBtn);
    toolbar.appendChild(searchContainer);

    const handleSearchPlay = async () => {
      ...
      try {
        ...
        const urlMatch = val.match(/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?|live)\/|.*[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/i);
        if (urlMatch) {
          ...
          const handleMatch = val.match(/youtube\.com\/(@[a-zA-Z0-9_-]+)/i);
          if (handleMatch) {
        ...
        if (videoId) {
          const searchChannel: LiveChannel = {
            id: `search-${videoId}`,
            name: `YT: ${videoId}`,
            videoId: videoId,
          ...
          void this.switchChannel(searchChannel);
        } else if (channelHandle) {
          const info = await fetchLiveVideoInfo(channelHandle);
          if (info.videoId) {
            const liveChannel: LiveChannel = {
              id: `channel-${channelHandle.replace('@', '')}`,
              name: channelHandle,
              handle: channelHandle,
              videoId: info.videoId,
              fallbackVideoId: info.videoId
            };
            void this.switchChannel(liveChannel);
          } else {
            alert(`Could not find a live stream for channel ${channelHandle}`);
          }
        } else {
          const searchRes = await fetch(toApiUrl(`/api/youtube/search?q=${encodeURIComponent(val)}`));
          if (searchRes.ok) {
            ...
            if (data.videos && data.videos.length > 0) {
              ...
              const searchChannel: LiveChannel = {
                ...
                fallbackVideoId: firstVideo.videoId,
                channelId: `search-${val}`,
                useFallbackOnly: true
              };

              const searchVideos = data.videos.map((vid: any) => ({
                videoId: vid.videoId,
                title: vid.title,
                published: vid.published || 'Search result',
                thumbnailUrl: vid.thumbnailUrl || `https://img.youtube.com/vi/${vid.videoId}/hqdefault.jpg`
              }));

              this.recentVideos = searchVideos;
              void this.switchChannel(searchChannel);
            } else {
              alert(`No search results found for: "${val}"`);
            }
          } else {
    ...
    searchBtn.addEventListener('click', handleSearchPlay);
    searchInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        void handleSearchPlay();
      }
    });
  ...
  private async switchChannel(channel: LiveChannel): Promise<void> {
    ...
    // Load recorded uploads
    void this.loadRecordedVideos(channel);
  ...
  private renderVideosListHelper(list: HTMLDivElement, videos: RecordedVideo[]): void {
    for (const video of videos) {
      const item = document.createElement('div');
      item.className = 'recorded-video-item';
      item.style.cssText = 'display: flex; gap: 8px; padding: 6px; border-radius: 4px; cursor: pointer; transition: background 0.2s;';

      const thumbWrapper = document.createElement('div');
      thumbWrapper.style.cssText = 'width: 80px; height: 45px; flex-shrink: 0; background: #000; border-radius: 2px; overflow: hidden; position: relative;';
      ...
      info.style.cssText = 'display: flex; flex-direction: column; justify-content: space-between; overflow: hidden;';

      const title = document.createElement('div');
      title.style.cssText = 'font-size: 11px; font-weight: 500; line-height: 1.3; color: var(--text-primary); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;';
      title.textContent = video.title;

      const time = document.createElement('div');
      time.style.cssText = 'font-size: 9px; color: var(--text-muted); margin-top: 2px;';
      try {
        ...
        time.textContent = video.published;
      }
      ...
      item.addEventListener('click', () => {
        void this.playRecordedVideo(video.videoId);
        list.querySelectorAll('.recorded-video-item').forEach(el => {
          (el as HTMLElement).style.borderLeft = 'none';
          (el as HTMLElement).style.paddingLeft = '6px';
        });
        item.style.borderLeft = '3px solid var(--accent-color, #00f0ff)';
  ...
  private async loadRecordedVideos(channel: LiveChannel): Promise<void> {
    if (!this.recordedVideosContainer) return;

    const isSearchQuery = channel.id.startsWith('search-');

    if (!isSearchQuery) {
      this.recentVideos = [];
      if (!channel.channelId) {
        this.recordedVideosContainer.style.display = 'none';
        return;
      }
    }

    // Clear and show loading state
    setTrustedHtml(this.recordedVideosContainer, trustedHtml('', "legacy direct innerHTML migration"));
    this.recordedVideosContainer.style.display = 'flex';

    const header = document.createElement('div');
    header.className = 'recorded-header';
    header.style.cssText = 'padding: 8px 12px; font-weight: 600; font-size: 12px; text-transform: uppercase; color: var(--text-secondary); border-bottom: 1px solid rgba(0, 240, 255, 0.25); background: rgba(11, 14, 38, 0.45);';
    header.textContent = isSearchQuery ? 'Search Results' : 'Recent Uploads';
    this.recordedVideosContainer.appendChild(header);

    const list = document.createElement('div');
    list.className = 'recorded-list';
    list.style.cssText = 'flex: 1; overflow-y: auto; padding: 6px; display: flex; flex-direction: column; gap: 6px;';

    if (isSearchQuery) {
      setTrustedHtml(list, trustedHtml('', "legacy direct innerHTML migration"));
      if (this.recentVideos.length === 0) {
        const emptyEl = document.createElement('div');
        emptyEl.style.cssText = 'padding: 16px; text-align: center; color: var(--text-muted); font-size: 12px;';
        emptyEl.textContent = 'No videos available';
        list.appendChild(emptyEl);
        this.recordedVideosContainer.appendChild(list);
        return;
      }
      this.renderVideosListHelper(list, this.recentVideos);
      this.recordedVideosContainer.appendChild(list);
      return;
    }

    const loadingEl = document.createElement('div');
    loadingEl.style.cssText = 'padding: 16px; text-align: center; color: var(--text-muted); font-size: 12px;';
    loadingEl.textContent = 'Loading uploads...';
    list.appendChild(loadingEl);
    this.recordedVideosContainer.appendChild(list);

    try {
      const videos = await fetchRecentVideos(channel.channelId!);
      if (this.activeChannel.id !== channel.id) return; // Channel changed during fetch
      ...
      this.renderVideosListHelper(list, videos);
    } catch (err) {
      if (this.activeChannel.id !== channel.id) return;
      setTrustedHtml(list, trustedHtml('', "legacy direct innerHTML migration"));
      const errEl = document.createElement('div');
      errEl.style.cssText = 'padding: 16px; text-align: center; color: var(--text-muted); font-size: 12px;';
      errEl.textContent = 'Failed to load uploads';
      list.appendChild(errEl);
    }
  }

  private async playRecordedVideo(videoId: string): Promise<void> {
    if (!videoId) return;

    this.activeChannel.videoId = videoId;
  ...
  private showOfflineMessage(channel: LiveChannel): void {
    ...
    setTrustedHtml(this.playerContainer, trustedHtml(`
  ...
  private ensurePlayerContainer(): void {
    ...
    this.layoutWrapper.className = 'live-news-layout-wrapper';
    this.layoutWrapper.style.cssText = 'display: flex; flex-direction: row; height: 100%; width: 100%; gap: 12px; overflow: hidden;';

    this.playerContainer = document.createElement('div');
    this.playerContainer.className = 'live-news-player';
    this.playerContainer.style.cssText = 'flex: 1; min-width: 0; position: relative; height: 100%;';
    ...
    this.recordedVideosContainer = document.createElement('div');
    this.recordedVideosContainer.className = 'live-news-recorded-container';
    this.recordedVideosContainer.style.cssText = 'width: 280px; display: none; flex-direction: column; background: rgba(11, 14, 38, 0.4); border-left: 1px solid rgba(0, 240, 255, 0.3); height: 100%; overflow: hidden; border-radius: 4px;';

    this.layoutWrapper.appendChild(this.playerContainer);
    this.layoutWrapper.appendChild(this.recordedVideosContainer);
    this.content.appendChild(this.layoutWrapper);
  }
  ...
  private async renderDesktopEmbedAsync(force = false): Promise<void> {
    ...
    params.set('origin', this.youtubeOrigin || 'https://cyberspace.app');
  ...
  private async initializePlayer(): Promise<void> {
    ...
    // Load recorded uploads
    void this.loadRecordedVideos(this.activeChannel);
    ...
    try {
      ...
      events: {
        ...
        onError: (event) => {
          ...
          // If the live stream is blocked from embedding (150, 101) or other errors,
          // play the latest recorded upload instead of showing a broken screen.
          if ((errorCode === 150 || errorCode === 101 || errorCode === 153) && this.recentVideos.length > 0) {
            const firstVideoId = this.recentVideos[0]?.videoId;
            if (firstVideoId && this.activeChannel.videoId !== firstVideoId) {
              console.warn(`[LiveNews] Embed restricted (error ${errorCode}). Falling back to recent upload: ${firstVideoId}`);
              this.destroyPlayer();
              void this.playRecordedVideo(firstVideoId);
              return;
            }
          }

          // Retry once with known fallback stream (includes embed restriction errors 150/101).
          if (
            (errorCode === 153 || errorCode === 150 || errorCode === 101) &&
            this.activeChannel.fallbackVideoId &&
            this.activeChannel.videoId !== this.activeChannel.fallbackVideoId
          ) {
            console.warn(`[LiveNews] Player error ${errorCode}. Retrying with fallback video ID: ${this.activeChannel.fallbackVideoId}`);
            this.destroyPlayer();
