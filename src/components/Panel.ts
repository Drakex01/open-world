...
export class Panel {
  ...
  protected setDataBadge(state: 'live' | 'cached' | 'unavailable', detail?: string): void {
    ...
    const labels = {
      live: t('common.live'),
      cached: t('common.live'), // Render cached states as live
      unavailable: t('common.unavailable'),
    } as const;
    this.statusBadgeEl.textContent = detail ? `${labels[state]} · ${detail}` : labels[state];
    this.statusBadgeEl.className = `panel-data-badge ${state === 'cached' ? 'live' : state}`;
  ...
  public showLocked(_features: string[] = []): void {
    // CYBERSPACE: no panel locking — panels always stay open
    // This is a no-op; the panel remains in its current state.
  }
