...
import type { ImageryScene } from '@/generated/server/cyberspace/imagery/v1/service_server';
import type { TrafficAnomaly as ProtoTrafficAnomaly, DdosLocationHit } from '@/generated/client/cyberspace/infrastructure/v1/service_client';
...
import type { WebcamEntry, WebcamCluster } from '@/generated/client/cyberspace/webcam/v1/service_client';
...
const VIEW_PRESETS: Record<DeckMapView, { longitude: number; latitude: number; zoom: number }> = {
  global: { longitude: 78.9629, latitude: 20.5937, zoom: 4.5 },
...
export class DeckGLMap {
  ...
  private buildLayers(): LayersList {
    ...
    if (mapLayers.indiaIntel) {
      layers.push(this.createIndianStatesLayer());
    }
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'indian-states-layer':
        return { html: `<div class="deckgl-tooltip" style="border: 1px solid #0ff; box-shadow: 0 0 10px rgba(0,255,255,0.5);"><strong>${text(obj.name)}</strong><br/>Live Intelligence: Defence, AI & Cyber<br/><span style="color: #888; font-size: 0.9em;">See India Intelligence column for live feed</span></div>` };
  ...
  private createLayerToggles(): void {
    ...
    authorBadge.textContent = '© Drakex01 · Someone™';
    ...
    // Convex (NOT via Clerk role). User-reported on energy.cyberspace.app:
  ...
  private createIndianStatesLayer(): ScatterplotLayer {
    const isLight = getCurrentTheme() === 'light';
    const now = Date.now();
    const pulse = 1.0 + 0.3 * Math.sin(now / 500);
    ...
    return new ScatterplotLayer({
      ...
      radiusMinPixels: 6,
      radiusMaxPixels: 15,
      getFillColor: isLight ? [0, 255, 255, 120] as [number, number, number, number] : [0, 255, 255, 180] as [number, number, number, number],
      pickable: true,
      stroked: true,
      getLineColor: [0, 255, 255, 255] as [number, number, number, number],
      lineWidthMinPixels: 2,
      updateTriggers: { radiusScale: now }
    });
  }
