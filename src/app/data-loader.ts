...
import type { ListFeedDigestResponse } from '@/generated/client/cyberspace/news/v1/service_client';
import type { GetSectorSummaryResponse, ListMarketQuotesResponse, ListCommodityQuotesResponse } from '@/generated/client/cyberspace/market/v1/service_client';
import type { SectorValuation } from '@/components/MarketPanel';
import { mountCommunityWidget } from '@/components/CommunityWidget';
import { ResearchServiceClient } from '@/generated/client/cyberspace/research/v1/service_client';
import {
  MarketPanel,
  StockAnalysisPanel,
  StockBacktestPanel,
...
import type { NewsItem as ProtoNewsItem, ThreatLevel as ProtoThreatLevel } from '@/generated/client/cyberspace/news/v1/service_client';
...
export class DataLoaderManager implements AppModule {
  ...
  private isPerFeedFallbackEnabled(): boolean {
    ...
    // Local dev: server digest relies on seeded Redis data that isn't
    // available locally. Always enable per-feed fallback so news panels
    // fetch RSS feeds directly instead of showing "No news available".
  ...
  async loadStockAnalysis(): Promise<void> {
    ...
    try {
      ...
      panel.showError('Stock analysis data unavailable — add a FINNHUB_API_KEY to .env.local to enable.');
    }
  }
  ...
  async loadStockBacktest(): Promise<void> {
    ...
    try {
      ...
      panel.showError('Backtesting data unavailable — add a FINNHUB_API_KEY to .env.local to enable.');
    }
  }
  ...
  private async _collectRegimeContext(): Promise<RegimeMacroContext | undefined> {
    try {
      ...
      const { MarketServiceClient } = await import('@/generated/client/cyberspace/market/v1/service_client');
  ...
  private async _collectYieldCurveContext(): Promise<YieldCurveContext | undefined> {
    try {
      const { EconomicServiceClient } = await import('@/generated/client/cyberspace/economic/v1/service_client');
      const { getRpcBaseUrl } = await import('@/services/rpc-client');
  ...
  async loadForecasts(): Promise<void> {
    try {
      const hydrated = getHydratedData('forecasts') as { predictions?: import('@/generated/client/cyberspace/forecast/v1/service_client').Forecast[]; generatedAt?: number } | undefined;
  ...
  async loadIntelligenceSignals(): Promise<void> {
    ...
    const hydratedUcdp = getHydratedData('ucdpEvents') as import('@/generated/client/cyberspace/conflict/v1/service_client').ListUcdpEventsResponse | undefined;
  ...
  async loadEconomicStress(): Promise<void> {
    try {
      ...
      const hydrated = getHydratedData('economicStress') as import('@/generated/client/cyberspace/economic/v1/service_client').GetEconomicStressResponse | undefined;
