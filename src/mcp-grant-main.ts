 ...
 *      `https://api.cyberspace.app/oauth/authorize-pro?...` — the
...
try {
  const savedTheme = localStorage.getItem('cyberspace-theme');
...
async function onAuthorizeClick(nonce: string): Promise<void> {
  ...
  // Defense-in-depth: the apex page MUST navigate only to api.cyberspace.app.
  ...
  if (target.origin !== 'https://api.cyberspace.app') {
