...
const FREE_MAP_PANEL_ACCESS_KEY = 'cyberspace-free-map-panel-access-v1';
...
export class App {
  ...
  constructor(containerId: string) {
    ...
    const storedVariant = localStorage.getItem('cyberspace-variant');
    ...
    if (storedVariant !== currentVariant) {
      ...
      const PANEL_KEY_RENAMES_MIGRATION_KEY = 'cyberspace-panel-key-renames-v2.6.8';
      ...
      const UNIFIED_MIGRATION_KEY = 'cyberspace-unified-panels-v1';
      ...
      const HAPPY_PANEL_FIX_KEY = 'cyberspace-happy-panel-fix-v1';
      ...
      const PANEL_ORDER_MIGRATION_KEY = 'cyberspace-panel-order-v1.9';
      ...
      if (currentVariant === 'tech') {
        const TECH_INSIGHTS_MIGRATION_KEY = 'cyberspace-tech-insights-top-v1';
    ...
    const PANEL_PRUNE_KEY = 'cyberspace-panel-prune-v1';
    ...
    const LAYOUT_RESET_MIGRATION_KEY = 'cyberspace-layout-reset-v2.5';
    ...
    if (currentVariant === 'full') {
      const baseKey = 'cyberspace-sources-reduction-v3';
      ...
      // first run with `userLang='en'` sets `cyberspace-locale-boost-en` and
      ...
      const localeKey = `cyberspace-locale-boost-${userLang}`;
  ...
  public async init(): Promise<void> {
    ...
    // on commodity.cyberspace.app: Trade Policy panel stuck at "Loading…"
  ...
  private enforceFreeTierLimits(): void {
    ...
    // CYBERSPACE: isProUser() always returns true — no cap enforcement needed
    if (isProUser()) return;
