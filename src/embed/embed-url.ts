...
export function buildEmbedMapUrl(
  ...
  const url = new URL(baseUrl, 'https://www.cyberspace.app');
...
export function buildEmbedIframeSnippet(url: string, options: { width?: string; height?: string } = {}): string {
  ...
  return `<iframe src="${escapeAttribute(url)}" title="CYBERSPACE live map" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="width:${width};height:${height}px;border:0;display:block" allowfullscreen></iframe>`;
}
