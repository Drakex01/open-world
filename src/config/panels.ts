...
const FULL_PANELS: Record<string, PanelConfig> = {
  ...
  'network-activity': { name: 'Network Activity', enabled: true, priority: 1 },
  'search-news': { name: 'Search News', enabled: true, priority: 1 },
  ...
  forecast: { name: 'AI Forecasts', enabled: true, priority: 1 }, // trial: unlocked on web, locked on desktop
  ...
  'india-intel': { name: 'India Intelligence', enabled: true, priority: 1 },
...
// ============================================
// ENERGY variant — energy.cyberspace.app
...
export function isPanelEntitled(_key: string, _config: PanelConfig, _isPro = false): boolean {
  // CYBERSPACE: all panels free for everyone
  return true;
}
...
export function enforceFreePanelLimit(
  ...
  // CYBERSPACE: no panel cap — everyone gets all panels
  const next: Record<string, PanelConfig> = {};
  for (const [key, config] of Object.entries(panelSettings)) {
    next[key] = { ...config };
  }
...
export const PANEL_CATEGORY_MAP: Record<string, { labelKey: string; panelKeys: string[]; variants?: string[] }> = {
  ...
  regionalNews: {
    ...
    panelKeys: ['india-intel', 'politics', 'us', 'europe', 'middleeast', 'africa', 'latam', 'asia'],
    variants: ['full'],
  },
  marketsFinance: {
