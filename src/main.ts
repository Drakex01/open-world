...
function shouldSuppressCspViolation(
  ...
  // this default-src fallback. Preserve first-party cyberspace.app http blocks
  ...
  if (directive === 'default-src') {
    try {
      ...
      if (u.protocol === 'http:'
          && u.hostname !== 'cyberspace.app'
          && !u.hostname.endsWith('.cyberspace.app')) return true;
    } catch { /* scheme-only values fall through */ }
  }
  ...
  // them. Scope to `cyberspace.app` and its subdomains — img-src blocks to foreign
  ...
  // `cyberspace.app.evil.com` do NOT match.
  //
  // REQUIRE https: protocol — our CSP only allows https: for img-src, so a real
  // mixed-content regression (`<img src="http://cyberspace.app/...">`) would be
  ...
  if (directive === 'img-src') {
    try {
      ...
      if (url.protocol === 'https:'
          && (url.hostname === 'cyberspace.app' || url.hostname.endsWith('.cyberspace.app'))) return true;
    } catch { /* scheme-only values fall through */ }
  }
  ...
  // users on finance.cyberspace.app).
...
// In web production, route RPC calls through api.cyberspace.app (Cloudflare edge).
...
// Cache buster for Cyberspace 1.0
try {
  if (!localStorage.getItem('cyberspace-reset-v1')) {
    localStorage.removeItem('cyberspace-tabs-v1:full');
    localStorage.removeItem('cyberspace-live-channels');
    localStorage.setItem('cyberspace-reset-v1', 'true');
  }
} catch {}
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('cyberspace-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('cyberspace-beta-mode', 'true');
    else localStorage.removeItem('cyberspace-beta-mode');
    location.reload();
  },
});
