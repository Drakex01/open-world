...
export function exportToJSON(data: ExportData, filename = 'cyberspace-export'): void {
  const jsonStr = JSON.stringify(sanitizeData(data), null, 2);
  downloadFile(jsonStr, `${filename}.json`, 'application/json');
}

export function exportToCSV(data: ExportData, filename = 'cyberspace-export'): void {
...
export class ExportPanel {
  ...
  private export(format: ExportFormat): void {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `cyberspace-${timestamp}`;
