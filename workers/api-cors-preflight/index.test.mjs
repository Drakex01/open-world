...
test('isAllowedOrigin accepts Vercel preview deploys under the dipteshwm team scope (mirrors api/_cors.js)', () => {
  // The project deploys previews under the "dipteshwm" Vercel team scope, so URLs
  // end in `-dipteshwm.vercel.app` (git-branch alias AND hash deployment forms).
  // The Worker MUST mirror api/_cors.js exactly — if it stays narrower, dipteshwm
  ...
  assert.equal(isAllowedOrigin('https://worldmonitor-git-feat-x-dipteshwm.vercel.app'), true);
  assert.equal(isAllowedOrigin('https://worldmonitor-r6q9o-dipteshwm.vercel.app'), true);
  // Tight allowlist: a foreign team scope, a non-worldmonitor app, and the
  // retired personal scope (worldmonitor-*-diptesh-<hash>, migration complete)
  ...
  assert.equal(isAllowedOrigin('https://some-other-app-dipteshwm.vercel.app'), false);
  assert.equal(isAllowedOrigin('https://worldmonitor-abc-diptesh-habib.vercel.app'), false);
});
