...
Cloudflare Worker bound to `api.cyberspace.app/*`. Owns CORS at the edge:
...
Three CORS surfaces sit in front of every browser request to `api.cyberspace.app`:
...
- `CLOUDFLARE_API_TOKEN` — token with `Workers Scripts:Edit` + `Workers
  Routes:Edit` for the `cyberspace.app` zone.
...
`~/.claude/skills/cyberspace-architecture-gotchas/reference/cloudflare-worker-overrides-vercel-cors-for-preflight.md`
