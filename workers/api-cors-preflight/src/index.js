...
//
// Bound to: api.cyberspace.app/*
// Source of truth for CORS on api.cyberspace.app. Short-circuits OPTIONS
...
//      ~/.claude/skills/cyberspace-architecture-gotchas/reference/
...
const ALLOWED_ORIGIN_PATTERNS = [
  ...
  // Vercel previews under the "Drakex01wm" team scope, e.g.
  //   cyberspace-git-<branch>-Drakex01wm.vercel.app / cyberspace-<hash>-Drakex01wm.vercel.app
  // Mirror of api/_cors.js + server/cors.ts (see superset note above).
  /^https:\/\/cyberspace-[a-z0-9-]+-Drakex01wm\.vercel\.app$/,
...
//     cyberspace.app fallback echo and get blocked by the browser).
...
export function buildCorsHeaders(origin) {
  const allowOrigin = isAllowedOrigin(origin) ? origin : 'https://cyberspace.app';
  return {
...
export default {
  async fetch(request) {
    ...
    // cyberspace.app fallback origin echo and the browser would block.
