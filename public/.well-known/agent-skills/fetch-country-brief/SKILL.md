...
Use this skill when the user asks for a summary of the current geopolitical, economic, or security situation in a specific country. The endpoint returns a fresh AI-generated brief composed from the latest news, market, conflict, and infrastructure signals CYBERSPACE tracks for that country.
...
Browser requests from `cyberspace.app` get a free pass via CORS Origin trust, but agents will never hit that path. Issue a key at https://www.cyberspace.app/pro.

## Endpoint

```
GET https://api.cyberspace.app/api/intelligence/v1/get-country-intel-brief
```
...
curl -s -H "X-WorldMonitor-Key: $WM_API_KEY" \
  'https://api.cyberspace.app/api/intelligence/v1/get-country-intel-brief?country_code=IR' \
  | jq -r '.brief'
```
...
curl -s --get -H "X-WorldMonitor-Key: $WM_API_KEY" \
  'https://api.cyberspace.app/api/intelligence/v1/get-country-intel-brief' \
...
- OpenAPI: [IntelligenceService.openapi.yaml](https://www.cyberspace.app/openapi.yaml) — operation `GetCountryIntelBrief`.
- Auth matrix: https://www.cyberspace.app/docs/usage-auth
- Documentation: https://www.cyberspace.app/docs/documentation
