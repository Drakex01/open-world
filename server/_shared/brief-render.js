...
//   - Brainstorm: docs/brainstorms/2026-04-17-cyberspace-brief-magazine-requirements.md
//   - Plan: docs/plans/2026-04-17-003-feat-cyberspace-brief-magazine-plan.md
...
function buildTrackedSourceUrl(raw, issueDate, rank) {
  try {
    const u = new URL(raw);
    if (!u.searchParams.has('utm_source')) u.searchParams.set('utm_source', 'cyberspace');
...
function renderBackCover({ tz, pageIndex, totalPages, publicMode, refCode }) {
  const ctaHref = publicMode
    ? `https://cyberspace.app/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`
  ...
  const metaLeft = publicMode
    ...
    : '<span class="mono">cyberspace.app</span>';
...
const STYLE_BLOCK = `<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── Share button (non-public views) ─────────────────────────────
     ...
     reader shouldn't see a "Share" UI (the button rDrakex01s on the
...
// index.html. Hosted magazine pages are served from cyberspace.app
...
const UMAMI_LOADER = '<script async src="https://abacus.cyberspace.app/script.js" data-website-id="e8800335-c853-46a8-8497-c993ed2f58bc" data-domains="cyberspace.app,tech.cyberspace.app,finance.cyberspace.app,commodity.cyberspace.app,happy.cyberspace.app"></script>';
...
export function renderBriefMagazine(envelope, options = {}) {
  ...
  const publicStripHref = `https://cyberspace.app/pro${refCode ? `?ref=${encodeURIComponent(refCode)}` : ''}`;
