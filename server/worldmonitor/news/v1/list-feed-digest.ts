...
} from '../../../../src/generated/server/cyberspace/news/v1/service_server';
...
function generateMockParseResult(feed: ServerFeed, _variant: string, category: string): ParseResult {
  ...
  const categoryHeadlines: Record<string, string[]> = {
    politics: [
      'Global Summit: Leaders reach consensus on economic cooperation framework',
      'Elections Update: High turnout reported in pivotal regional elections',
      'Policy Reform: New environmental guidelines announced for next decade',
      'Diplomatic Relations: Direct talks resume between major regional powers',
    ],
    us: [
      'Infrastructure Project: Massive public works funding bill passes Senate',
      'Labor Report: Strong job growth figures outperform analyst forecasts',
      'Tech Hearing: Congress reviews antitrust policies on digital marketplaces',
      'Healthcare Expansion: Pilot program launched for rural medical clinics',
    ],
    europe: [
      'EU Accord: New cross-border digital services guidelines approved',
      'Energy Transition: Wind power generation reaches record share in North Sea',
      'Trade Agreements: Ministers finalize bilateral export adjustments',
      'Heritage Protection: Cultural restoration initiatives receive public backing',
    ],
    tech: [
      'Product Launch: Breakthrough quantum processor announced by research lab',
      'Startup Funding: Biotech venture secures record investment for clinical trials',
      'Cyber Security: Major operating system patches zero-day vulnerability',
      'Hardware Shortage: Supply chain delays ease for next-generation silicon',
    ],
    ai: [
      'Model Release: Open-source LLM matches frontier models in reasoning bench',
      'Ethics Guidelines: Consortium proposes standards for AI agent autonomy',
      'AI Hardware: Next-generation cluster achieves sub-millisecond latency',
      'Robotics Milestone: Neural net-driven humanoid demonstrates fine motor skills',
    ],
    gov: [
      'Department Statement: Agency outlines new regulations on critical infrastructure',
      'Public Report: Economic census details shift in metropolitan employment',
      'Treasury Advisory: Financial stability council updates macro-prudential guidelines',
      'CISA Alert: Cybersecurity warning issued for emergency systems',
    ],
    thinktanks: [
      'CSIS Analysis: Report maps supply chain bottlenecks in critical minerals',
      'Atlantic Council: Geopolitical implications of emerging regional trade blocs',
      'Foreign Affairs: Analyzing the future of global maritime logistics and security',
      'War on the Rocks: Tactical lessons from recent defensive operations in Europe',
    ],
  };

  const genericHeadlines = [
    'Strategic Outlook: Key milestones achieved in international cooperation',
    'Special Report: Industrial modernization trends shaping next fiscal year',
    'Weekly Digest: Important regulatory changes and updates for sector professionals',
    'Field Assessment: Local communities implement resource management programs',
  ];

  const headlines = categoryHeadlines[category] || genericHeadlines;

  for (let i = 0; i < count; i++) {
    const title = `${feed.name}: ${headlines[i % headlines.length]} [Mock]`;
    const publishedAt = now - i * 2 * 60 * 60 * 1000 - Math.floor(Math.random() * 60 * 60 * 1000);
    items.push({
      source: feed.name,
      title,
      link: `${feed.url}?mock=${i}`,
      publishedAt,
      ...
      confidence: 95,
      classSource: 'keyword',
      importanceScore: 60 + Math.floor(Math.random() * 25),
      corroborationCount: 1 + Math.floor(Math.random() * 3),
...
async function fetchAndParseRss(
  ...
  try {
    ...
    let text = await fetchRssText(feed.url, signal).catch((err) => {
      console.error(`[feed-fetch-error] Failed to fetch ${feed.url}:`, err);
      return null;
    });
    ...
    if (!text) {
      if (process.env.LOCAL_API_MODE === 'true') {
        const mockResult = generateMockParseResult(feed, variant, category);
        await setCachedJson(cacheKey, mockResult, CACHE_TTL_HEALTHY_S);
        return mockResult;
      }
    ...
    for (let i = 0; i < allEntries.length; i += BATCH_CONCURRENCY) {
      ...
      const settled = await Promise.allSettled(
        batch.map(async ({ category, feed }) => {
          const result = await fetchAndParseRss(feed, variant, deadlineController.signal, category);
