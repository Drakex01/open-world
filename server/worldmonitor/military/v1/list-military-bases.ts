import type {
  ...
  MilitaryBaseEntry,
  MilitaryBaseCluster,
} from '../../../../src/generated/server/cyberspace/military/v1/service_server';
...
function getLocalBasesFallback(
  ...
  try {
    ...
    if (!filePath) {
      console.warn('[list-military-bases] Local bases file not found on disk at any expected path.');
      return { bases: [], clusters: [], totalInView: 0, truncated: false };
    }

    const raw = readFileSync(filePath, 'utf-8');
    const allBases = JSON.parse(raw) as any[];
    if (!Array.isArray(allBases)) return { bases: [], clusters: [], totalInView: 0, truncated: false };

    const antimeridian = swLon > neLon;
    const bases: MilitaryBaseEntry[] = [];

    for (const meta of allBases) {
      const lat = Number(meta.lat) || 0;
      const lon = Number(meta.lon) || 0;

      // Coordinate boundary check
      if (lat < swLat || lat > neLat) continue;
      if (antimeridian) {
        if (lon < swLon && lon > neLon) continue;
      } else {
        if (lon < swLon || lon > neLon) continue;
      }
    ...
    console.error('[list-military-bases] Local fallback failed:', err);
    return { bases: [], clusters: [], totalInView: 0, truncated: false };
  }
}
...
export async function listMilitaryBases(
  ...
  try {
    ...
    try {
      activeVersion = await getCachedJson('military:bases:active') as string | null;
      if (!activeVersion) {
        activeVersion = await getCachedJson('military:bases:active', true) as string | null;
        rawKeys = true;
      }
    } catch (err) {
      console.warn('[list-military-bases] Redis query failed, falling back to local file:', err);
    }

    if (!activeVersion) {
      console.info('[list-military-bases] Redis key missing, using local bases file fallback');
      setResponseHeader(ctx.request, 'X-Bases-Debug', 'local-fallback');
      return getLocalBasesFallback(swLat, swLon, neLat, neLon, zoom, typeFilter, kindFilter, countryFilter);
    }
    ...
    try {
      ...
      console.warn('[list-military-bases] Redis query failed, using local fallback:', err);
      setResponseHeader(ctx.request, 'X-Bases-Debug', 'local-fallback-on-error');
      return getLocalBasesFallback(swLat, swLon, neLat, neLon, zoom, typeFilter, kindFilter, countryFilter);
    }
  } catch (outerErr) {
