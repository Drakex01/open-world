...
All notable changes to CYBERSPACE are documented here.
...
- **Sebuf API migration (#3207)** — scenario + supply-chain endpoints migrated to the typed sebuf contract. RPC URLs now derive from method names; the five renamed v1 URLs remain live as thin aliases so existing integrations keep working:
  ...
  Aliases will retire at the next v1→v2 break ([#3282](https://github.com/Drakex01/cyberspace/issues/3282)).
...
- CDN-Cache-Control header now only set for trusted origins (cyberspace.app, Vercel previews, Tauri); no-origin server-side requests always reach the edge function so `validateApiKey` can run, closing a potential cache-bypass path for external scrapers
...
- **Edge caching**: Cloudflare edge caching for proxy.cyberspace.app (#478) and api.cyberspace.app (#471)
...
- **Finance variant**: Added a dedicated market-first variant (`finance.cyberspace.app`) with finance/trading-focused feeds, panels, and map defaults
...
- **CORS hardening**: Tighten Vercel preview deployment regex to block origin spoofing (`cyberspaceEVIL.vercel.app`)
