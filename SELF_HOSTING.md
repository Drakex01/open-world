# 🌍 Self-Hosting CYBERSPACE

Run the full CYBERSPACE stack locally with Docker/Podman.
...
*/30 * * * * cd /path/to/cyberspace && ./scripts/run-seeders.sh >> /tmp/wm-seeders.log 2>&1
```
...
|-----------|---------|------|
| `cyberspace` | nginx + Node.js API (supervisord) | 3000 → 8080 |
| `cyberspace-redis` | Data store | 6379 (internal) |
| `cyberspace-redis-rest` | Upstash-compatible REST proxy | 8079 |
| `cyberspace-ais-relay` | Live vessel tracking WebSocket | 3004 (internal) |
...
# Full Docker image
docker build -t cyberspace:latest -f Dockerfile .
...
| 🔴 nginx won't start | Check `podman logs cyberspace` — likely missing `gettext` package |
...
| 🚢 No vessel data | Set `AISSTREAM_API_KEY` in both `cyberspace` and `ais-relay` services |
