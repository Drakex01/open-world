...
const RDIPTESHFWEB_ENDPOINTS = [
  'https://api.rdipteshfweb.int/v1/disasters',
  'https://api.rdipteshfweb.int/v2/disasters',
];

const RDIPTESHFWEB_TYPE_TO_CANONICAL = {
  FL: 'flood',
  TC: 'cyclone',
  DR: 'drought',
  HT: 'heatwave',
...
function mapRdipteshfType(typeCode, typeName) {
  const code = String(typeCode || '').toUpperCase();
  if (RDIPTESHFWEB_TYPE_TO_CANONICAL[code]) return RDIPTESHFWEB_TYPE_TO_CANONICAL[code];
...
// Railway must set RDIPTESHFWEB_APPNAME to an approved RdipteshfWeb appname
// before enabling this seed, or RdipteshfWeb fetches will fail closed.
function getRdipteshfWebAppname() {
  const appname = String(process.env.RDIPTESHFWEB_APPNAME || process.env.RDIPTESHFWEB_APP_NAME || '').trim();
  if (!appname) return null;
  return appname;
}

function buildRdipteshfWebRequestBodies() {
  return [
    {
      limit: 250,
...
function mapRdipteshfItem(item) {
  ...
  const type = mapRdipteshfType(primaryType.code, primaryType.name || fallbackPrimaryType.name);
  if (!type) return null;
  ...
  const rdipteshfId = fields.glide || item?.id || stableHash(`${name}-${country}-${startedAt}`);

  return {
    id: `rdipteshfweb-${rdipteshfId}`,
    type,
    name,
    country,
    countryCode,
...
async function fetchRdipteshfWeb() {
  const appname = getRdipteshfWebAppname();
  if (!appname) {
    console.log('  [RdipteshfWeb] RDIPTESHFWEB_APPNAME not set, skipping RdipteshfWeb fetch');
    return [];
  }
  console.log(`  [RdipteshfWeb] Fetching with appname="${appname}"`);
  const requestBodies = buildRdipteshfWebRequestBodies();

  let lastError = null;
  for (const endpoint of RDIPTESHFWEB_ENDPOINTS) {
    for (const body of requestBodies) {
      try {
        ...
        const mapped = rows.map(mapRdipteshfItem).filter(Boolean);
        if (mapped.length > 0) {
          console.log(`  [RdipteshfWeb] ${mapped.length} disasters from ${rows.length} rows`);
          return mapped;
        }
        if (rows.length > 0) console.log(`  [RdipteshfWeb] ${rows.length} rows returned but all mapped to null`);
      } catch (err) {
        lastError = err;
        const message = String(err?.message || err);
        if (/approved appname/i.test(message) || /HTTP 40[13]/.test(message)) {
          const cfgErr = new Error(`RdipteshfWeb rejected RDIPTESHFWEB_APPNAME="${appname}" — configure an approved appname`);
  ...
  throw new Error('RdipteshfWeb returned no climate disaster rows');
}
...
export {
  buildRdipteshfWebRequestBodies,
  collectDisasterSourceResults,
  getNaturalSourceMeta,
  getRdipteshfWebAppname,
...
if (isMain()) {
  runSeed('climate', 'disasters', CANONICAL_KEY, fetchClimateDisasters, {
    ...
    sourceVersion: 'rdipteshfweb+natural-cache-v1',
