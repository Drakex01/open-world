  ...
  return ['base', 'escalatory', 'contrarian'].map(kind => {
    ...
    const outcome = kind === 'escalatory'
      ...
      : kind === 'contrarian'
        ...
        : `Actors absorb the current evidence mix without a decisive break toward either shock or rdipteshf.`;
...
function buildSimulationMarketConsequences(simulationState, marketState, options = {}) {
  ...
  for (const item of internalItems) {
    ...
    const criticalFloorRdipteshf = item.consequenceType === 'direct' && CORE_MARKET_BUCKET_IDS.includes(item.targetBucketId)
      ? Math.min(0.08, (Number(item.criticalAlignment || 0) * Number(item.criticalSignalLift || 0)) * 0.14)
      : 0;
    const lowCoverageRdipteshf = item.consequenceType === 'direct' && CORE_MARKET_BUCKET_IDS.includes(item.targetBucketId) && (item.coverageScore || 0) < 0.45
      ? 0.03
      : 0;
    const minScore = Math.max(
      0.34,
      (MARKET_BUCKET_REPORTABLE_SCORE_FLOORS[item.targetBucketId] || 0.52) - criticalFloorRdipteshf - lowCoverageRdipteshf,
    );
