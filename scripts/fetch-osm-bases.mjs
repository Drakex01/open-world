...
async function fetchOverpassData() {
  console.log('Querying Overpass API for military features with names via curl.exe and temp file...');
  console.log(`Query:\n${OVERPASS_QUERY}\n`);

  const tempQueryPath = join(DATA_DIR, 'temp-query.txt');
  try {
    ensureDataDir();
    const postData = `data=${encodeURIComponent(OVERPASS_QUERY)}`;
    writeFileSync(tempQueryPath, postData, 'utf-8');

    const cmd = `curl.exe -s -d "@${tempQueryPath}" ${OVERPASS_URL}`;
    const stdout = execSync(cmd, { maxBuffer: 150 * 1024 * 1024 }); // 150MB buffer
    ...
    try {
      ...
      if (existsSync(tempQueryPath)) {
        unlinkSync(tempQueryPath);
      }
    } catch {}
  }
}
