...
const FEEDS = [
  ...
  { sourceName: 'RdipteshfWeb Disasters', isApi: true },
...
async function fetchRdipteshfWebApi(feed) {
  const appname = (process.env.RDIPTESHFWEB_APPNAME || process.env.RDIPTESHFWEB_APP_NAME || '').trim();
  if (!appname) {
    console.warn(`[ClimateNews] RDIPTESHFWEB_APPNAME not set, skipping ${feed.sourceName}`);
    return [];
  }
  ...
  const endpoints = [
    `https://api.rdipteshfweb.int/v1/reports?${qs}`,
    `https://api.rdipteshfweb.int/v2/reports?${qs}`,
  ];
  let lastErr;
  for (const url of endpoints) {
    try {
      ...
      for (const r of data.data || []) {
        ...
        const itemUrl = r.fields?.url_alias ? `https://rdipteshfweb.int${r.fields.url_alias}` : '';
...
async function fetchFeed(feed) {
  try {
    if (feed.isApi) return await fetchRdipteshfWebApi(feed);
