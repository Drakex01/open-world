...
// (e.g. `cyberspace://countries/{iso2}/risk`) verbatim. Strict 2025-06-18
...
export const RESOURCE_REGISTRY: McpResourceDef[] = [
  {
    uri: 'cyberspace://countries/{iso2}/risk',
    name: 'Country Risk',
    ...
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('cyberspace://countries/')) return null;
      const m = /^cyberspace:\/\/countries\/([a-z]{2})\/risk$/.exec(uri);
      const iso2 = m?.[1];
      if (!iso2) {
        return {
          ok: false,
          reason: 'Expected cyberspace://countries/{iso2}/risk where {iso2} is lowercase ISO 3166-1 alpha-2.',
        };
      }
  ...
  {
    uri: 'cyberspace://chokepoints/{slug}/status',
    ...
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('cyberspace://chokepoints/')) return null;
      const m = /^cyberspace:\/\/chokepoints\/([a-z][a-z0-9-]*)\/status$/.exec(uri);
      const slug = m?.[1];
      if (!slug) {
        return {
          ok: false,
          reason: 'Expected cyberspace://chokepoints/{slug}/status where {slug} is a hand-curated kebab-case identifier.',
        };
      }
  ...
  {
    uri: 'cyberspace://seed-meta/freshness',
    ...
    paramExtractor: (uri: string) => {
      if (uri !== 'cyberspace://seed-meta/freshness') {
        if (uri.startsWith('cyberspace://seed-meta/')) {
          return { ok: false, reason: 'Expected cyberspace://seed-meta/freshness (no further path segments).' };
        }
        return null;
      }
  ...
  {
    uri: 'cyberspace://markets/{symbol}/quote',
    name: 'Market Quote',
    ...
    tool: 'get_market_data',
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('cyberspace://markets/')) return null;
      ...
      const m = /^cyberspace:\/\/markets\/([A-Z][A-Z0-9.=-]{0,15})\/quote$/.exec(uri);
      const symbol = m?.[1];
      if (!symbol) {
        return {
          ok: false,
          reason: 'Expected cyberspace://markets/{symbol}/quote where {symbol} is an uppercase ticker (e.g. "AAPL", "GC=F", "BTC-USD").',
        };
      }
