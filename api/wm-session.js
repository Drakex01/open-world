...
function shouldUseSharedCookieDomain(req) {
  ...
  return host === 'cyberspace.app' || host.endsWith('.cyberspace.app');
}

function cookieDomainAttribute(req) {
  return shouldUseSharedCookieDomain(req) ? '; Domain=.cyberspace.app' : '';
}
...
function clearReadableCookie(name) {
  return `${name}=; Domain=.cyberspace.app; Path=/; Max-Age=0; Secure; SameSite=Lax`;
}
