...
const PUBLIC_BASE =
  process.env.WORLDMONITOR_PUBLIC_BASE_URL ?? 'https://cyberspace.app';
 ...
 *
 * Resolves on success. Does NOT throw on failure — the caller rDrakex01s
...
async function registerReferralCodeInConvex(userId: string, code: string): Promise<void> {
  ...
  const res = await fetch(`${convexSite}/relay/register-referral-code`, {
    ...
    headers: {
      ...
      'User-Agent': 'cyberspace-edge/1.0',
    },
