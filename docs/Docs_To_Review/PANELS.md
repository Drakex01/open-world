...
> **CYBERSPACE** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-cyberspaceio)
5. [Tech Variant Panels](#5-tech-variant-panels-techcyberspaceio)
6. [Finance Variant Panels](#6-finance-variant-panels-financecyberspaceio)
...
CYBERSPACE uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | cyberspace.app | Geopolitical intelligence, OSINT, defense | 44 |
| `tech` | tech.cyberspace.app | Technology, AI/ML, startups, VC | Tech-focused |
| `finance` | finance.cyberspace.app | Markets, trading, macro, commodities | Finance-focused |
| `commodity` | commodity.cyberspace.app | Mining, metals, energy, supply chains | Commodity-focused |
| `happy` | happy.cyberspace.app | Positive news, breakthroughs, conservation | Happy-focused |
| `energy` | energy.cyberspace.app | Oil, gas, chokepoints, energy security | Energy-focused |
...
export const STORAGE_KEYS = {
  panels:        'cyberspace-panels',          // Panel visibility toggles
  monitors:      'cyberspace-monitors',         // Monitor keyword configs
  mapLayers:     'cyberspace-layers',           // Map layer toggles
  disabledFeeds: 'cyberspace-disabled-feeds',   // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `cyberspace-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `cyberspace-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `cyberspace-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `cyberspace-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (cyberspace.io)
...
## 5. Tech Variant Panels (tech.cyberspace.io)
...
## 6. Finance Variant Panels (finance.cyberspace.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `cyberspace-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `cyberspace-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `cyberspace-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `cyberspace-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `cyberspace-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `cyberspace-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`cyberspace-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:

```typescript
if (storedVariant !== currentVariant) {
  localStorage.setItem('cyberspace-variant', currentVariant);
...
This ensures users switching between variant domains (e.g. from cyberspace.io to tech.cyberspace.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to CYBERSPACE.
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('cyberspace-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('cyberspace-panels')
