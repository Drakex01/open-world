# Component Documentation — CYBERSPACE
...
CYBERSPACE's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['cyberspace-panel-spans']` as a JSON map of `{ [id]: height }`.
...
CYBERSPACE ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['cyberspace_recent_searches']`
(most recent 10).
