...
CYBERSPACE is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
```typescript
const PANEL_SPANS_KEY = 'cyberspace-panel-spans';
...
Stored in `localStorage` under `cyberspace-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `cyberspace-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `cyberspace-monitors` | `Monitor[]` | Color/label configs for monitors |
| `cyberspace-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `cyberspace-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('cyberspace-theme', 'light')
...
export async function initDB(): Promise<IDBDatabase> {
  // Opens or creates cyberspace_db v1
...
- **Toggles key:** `cyberspace-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by CYBERSPACE:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `cyberspace-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `cyberspace-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `cyberspace-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `cyberspace-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `cyberspace-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `cyberspace-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
| `cyberspace-disabled-feeds` | User-disabled news sources | `string[]` (JSON) | `STORAGE_KEYS.disabledFeeds` |
| `cyberspace-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `cyberspace-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `cyberspace-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `cyberspace-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
