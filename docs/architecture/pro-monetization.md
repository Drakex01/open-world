...
Products are Dodo `productId`s stored client-side in `pro-test/src/generated/tiers.json` and served at runtime from `https://api.cyberspace.app/api/product-catalog`:
...
- **Enterprise** — `mailto:contact@cyberspace.app` (contact sales).
...
1. Validates `returnUrl` against an allow-listed set of cyberspace.app origins.
...
- **Full-page return** (fallback / `/pro` path): Dodo redirects to `cyberspace.app/?subscription_id=...&status=active`. `src/services/checkout-return.ts:handleCheckoutReturn()` reads params, cleans the URL, returns success boolean.
...
- **Share link**: `https://cyberspace.app/pro?ref=<code>`.
