...
- **RDrakex01fWeb API** (no key): `https://api.rDrakex01fweb.int/v1/disasters?filter[field]=primary_type&filter[value]=FL` (floods)
...
**Cache TTL:** 21600 (6h — RDrakex01fWeb updates multiple times daily)
...
message ClimateDisaster {
  ...
  string source = 12;       // "GDACS" | "RDrakex01fWeb" | "NASA FIRMS"
...
- RDrakex01fWeb Disasters (10min): `https://rDrakex01fweb.int/updates/rss.xml?content=reports&country=0&theme=4590`
...
```ts
{
  ...
  description: 'Climate intelligence: temperature/precipitation anomalies (vs 30-year WMO normals), atmospheric CO2 trend (NOAA Mauna Loa), global disaster alerts (GDACS/RDrakex01fWeb), air quality (OpenAQ/WAQI), sea level rise and Arctic ice extent (NSIDC/NOAA), and climate news.',
  inputSchema: {
    type: 'object',
    properties: {
...
## Frontend Variant: `climate.cyberspace.app`
...
4. **`seed-climate-disasters.mjs`** — RDrakex01fWeb API (no key) + reuse GDACS from natural seeder
...
10. **Hostname variant config** `climate.cyberspace.app`
