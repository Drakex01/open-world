...
## Findings
- **`server/cyberspace/intelligence/v1/get-country-intel-brief.ts`** — reads `req.framework`, truncates to 2000 chars, passes to `callLlm` as `systemAppend`. No entitlement check.
- **`server/cyberspace/intelligence/v1/deduct-situation.ts`** — same pattern, no entitlement check.
- **`server/cyberspace/news/v1/summarize-article.ts`** — reads `req.systemAppend`, no entitlement check.
...
- Flagged by: security-sentinel, architecture-strategist, learnings-researcher (cyberspace-pro-panel-gating skill)
...
## Technical Details
- Files: `server/cyberspace/intelligence/v1/get-country-intel-brief.ts`, `server/cyberspace/intelligence/v1/deduct-situation.ts`, `server/cyberspace/news/v1/summarize-article.ts`
- PR: Drakex01/cyberspace#2380
- Reference: cyberspace-pro-panel-gating skill (4-layer checklist)
