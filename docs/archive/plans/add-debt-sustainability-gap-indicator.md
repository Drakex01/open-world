...
Scorer: `scoreFiscalSpace` in `server/cyberspace/resilience/v1/_dimension-scorers.ts:2013`.
...
### 2. `server/cyberspace/resilience/v1/_indicator-registry.ts`
...
### 3. `server/cyberspace/resilience/v1/_dimension-scorers.ts::scoreFiscalSpace`
...
Locate the type def (likely in `_indicator-registry.ts` or `server/cyberspace/resilience/types.ts`), add:
```ts
...
2. `server/cyberspace/resilience/v1/_indicator-registry.ts` — new indicator + rebalanced weights.
3. `server/cyberspace/resilience/v1/_dimension-scorers.ts::scoreFiscalSpace` — add 4th sub-score.
