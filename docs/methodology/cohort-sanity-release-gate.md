...
- **Every merge touching a scorer file** in `server/cyberspace/resilience/v1/_dimension-scorers.ts`,
  `server/cyberspace/resilience/v1/_shared.ts`, or a scorer-feeding
...
API_BASE=https://api.cyberspace.app \
...
| Var | Default | Notes |
|---|---|---|
| `API_BASE` | (required unless FIXTURE set) | e.g. `https://api.cyberspace.app` |
