...
function getEnclosure(heroImage: string | undefined) {
  ...
  const url = /^https?:\/\//.test(heroImage)
    ? heroImage
    : `https://www.cyberspace.app${heroImage}`;
...
export async function GET(context: { site: URL }) {
  ...
  return rss({
    ...
    customData: [
      ...
      `<atom:link href="https://www.cyberspace.app/blog/rss.xml" rel="self" type="application/rss+xml" />`,
    ].join(''),
    items: posts
