---
title: "Build on CYBERSPACE: APIs and Developer Platform"
description: "Build intelligence apps on CYBERSPACE's typed API: 34 services, 276 proto files, 60+ edge functions, and auto-generated TypeScript clients. AGPL-3.0."
metaTitle: "Developer API & Open Source Platform | CYBERSPACE"
...
heroImage: "/blog/images/blog/build-on-cyberspace-developer-api-open-source.jpg"
pubDate: "2026-03-09"
---
...
CYBERSPACE is designed differently. The entire intelligence platform, every data feed, every scoring algorithm, every aggregation pipeline, is built on a **typed API layer** that developers can use, extend, and build upon.

## Proto-First Architecture

CYBERSPACE uses **Protocol Buffers (protobuf)** as the single source of truth for all API contracts. The codebase contains:
...
For developers building on CYBERSPACE, this means you can trust the API contracts completely. If the proto says a field is `int64`, it's `int64`. If it says `repeated string`, it's an array of strings.

## 34 Service Domains

CYBERSPACE's API is organized into domain-specific services:
...
```
api.cyberspace.app/api/{domain}/v1/{rpc}
```

For example:

- `api.cyberspace.app/api/market/v1/quotes` for stock quotes
- `api.cyberspace.app/api/conflict/v1/events` for conflict data
- `api.cyberspace.app/api/intelligence/v1/cii` for Country Instability Index scores

## Building with CYBERSPACE's API
...
Feed CYBERSPACE data into your own analytics:
...
- Build custom scoring models using CYBERSPACE's raw data feeds

### Mobile Apps

Build a mobile app that consumes CYBERSPACE's API for a custom mobile intelligence experience. The OpenAPI spec makes it accessible from any language (Swift, Kotlin, Python, Go).
...
CYBERSPACE is AGPL-3.0. You can self-host the entire platform, including [local AI capabilities that run without cloud dependencies](/blog/posts/ai-powered-intelligence-without-the-cloud/):
...
For reference, CYBERSPACE is built with:
...
## Why Build on CYBERSPACE?

The intelligence industry has a consolidation problem. A handful of vendors control the data, the algorithms, and the interfaces. Analysts are locked into ecosystems they can't customize, audit, or extend. See how CYBERSPACE [compares to traditional intelligence tools](/blog/posts/cyberspace-vs-traditional-intelligence-tools/) in practice.

CYBERSPACE's open, typed, proto-first architecture is the alternative:
...
The intelligence platform of the future isn't a product. It's an ecosystem. CYBERSPACE is building the foundation.

Building an AI agent instead of an app? The same platform is exposed as a Model Context Protocol server with 39 live tools. See [how to connect Claude and other agents to CYBERSPACE's MCP server](/blog/posts/cyberspace-mcp-server-ai-agents-real-time-intelligence/).

## Frequently Asked Questions

**Is the CYBERSPACE API free to use?**
Yes. CYBERSPACE is AGPL-3.0 open source. You can use the public API at api.cyberspace.app or self-host the entire stack. There are no API keys required for public endpoints and no usage fees.
