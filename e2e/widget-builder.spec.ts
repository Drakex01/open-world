...
test.describe('AI widget builder', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript((key) => {
      if (!sessionStorage.getItem('__widget_e2e_init__')) {
        ...
        localStorage.setItem('cyberspace-variant', 'happy');
  ...
  test('creates a widget through the live modal flow and persists it after reload', async ({ page }) => {
    ...
    await installWidgetAgentMocks(
      ...
      [
        {
          delayMs: 250,
          endpoint: '/rpc/cyberspace.markets.v1.MarketsService/GetCommodities',
          title: 'Oil vs Gold',
          html: createHtml,
        },
      ],
  ...
  test('supports modify, keeps session history, exposes touch-sized controls, and cleans storage on delete', async ({ page }) => {
    ...
    await installWidgetAgentMocks(page, [
      ...
      {
        endpoint: '/rpc/cyberspace.aviation.v1.AviationService/GetAirportDelays',
    ...
    await page.evaluate((widgetId: string) => {
      localStorage.setItem('cyberspace-panel-spans', JSON.stringify({ [widgetId]: 2 }));
      localStorage.setItem('cyberspace-panel-col-spans', JSON.stringify({ [widgetId]: 3 }));
    }, storedWidgetMeta!.id);
    ...
    const cleanedStorage = await page.evaluate(() => {
      return {
        ...
        rowSpans: localStorage.getItem('cyberspace-panel-spans'),
        colSpans: localStorage.getItem('cyberspace-panel-col-spans'),
      };
    });
